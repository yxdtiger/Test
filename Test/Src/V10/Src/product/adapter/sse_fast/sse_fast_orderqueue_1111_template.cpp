...
wmdf::SSEFastOrderQueue1111Template::SSEFastOrderQueue1111Template()
...
wmdf::WindFastMessage* wmdf::SSEFastOrderQueue1111Template::DecodeRaw( uint8_t* raw_data,uint32_t len )
{
	...
	while(!fast_decoder_->Empty())
	{
		...
		assert(tid==SSE_ORDERQUEUE_1111_TYPE);
				...
				for (int i=0;i<current_Orderqueue.price_level_count;i++)
				{
					...
					if(priceLevel.orders_count>0)
					{
						...
						for (int j=0;j<priceLevel.orders_count;j++)
						{
							...
							orderitem.order_qty = fast_decoder_->DecodeInt32(3,DEFAULT);
