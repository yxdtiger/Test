...
namespace wmdf
{
  ...
  public:
    ...
    static std::string datetime2str (date_time_t datetime) {
      int32_t year, month, day,hour,minute,second,msecond ;
      ...
      sprintf(str,"%4d-%02d-%02d %02d:%02d:%02d.%03d",year,month,day,hour,minute,second,msecond);
    ...
    static date_t today()
    {
        ...
        ftime(&tp);
        tm = localtime (&(tp.time));
        return GDATE (tm->tm_year+1900,tm->tm_mon+1,tm->tm_mday);
      #endif
    }
    ...
    static date_time_t now()
    {
        ...
        ftime(&tp);
        tm = localtime (&(tp.time));
        return GDATETIME (tm->tm_year+1900,tm->tm_mon+1,tm->tm_mday, tm->tm_hour, tm->tm_min, tm->tm_sec, tp.millitm);
      #endif
    }
    ...
    {
      return DateTime::date2ymd(DateTime::today(),"-") +" "+ DateTime::time2hmsms(DateTime::get_current_time(),":");
    }
    ...
    static time_stamp_t hms2time(int32_t hms)
    {
      ...
      int32_t minute = (hms/100)%100;
      int32_t second = hms % 100;
      return hms2second(hour,minute,second);
    }
    static time_stamp_t hmsms2time(int32_t hmsms)
    {
      int32_t hour = hmsms/10000000;
      int32_t minute = (hmsms / 100000) % 100;
      int32_t second = (hmsms / 1000) % 100;
      int32_t millisecond = hmsms % 1000;
    ...
    {
      if(time==WMDF_CONSTS_32BIT_NIL_VALUE)
        return "";
      int32_t hour = time / 3600000;
      int32_t minute = time % 3600000 / 60000;
      int32_t second = time % 3600000 % 60000 / 1000;
