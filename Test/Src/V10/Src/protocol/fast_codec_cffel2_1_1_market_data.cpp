...
FastCodecCFFEL2_1_1_MarketData::FastCodecCFFEL2_1_1_MarketData()
{
	mainPMap=(Fast_PMap_T*)malloc(sizeof(struct Fast_PMap_T)+CFFEL2_1_1_MARKETDATA_PMAP_BITS*sizeof(uint8_t));
	memset(mainPMap,0,sizeof(struct Fast_PMap_T)+CFFEL2_1_1_MARKETDATA_PMAP_BITS*sizeof(uint8_t));
	mainPMap->size=CFFEL2_1_1_MARKETDATA_PMAP_BITS;

  main_DefaultValue = (TValue*)calloc(sizeof(struct tap_TValue), CFFEL2_1_1_MARKETDATA_PMAP_BITS);
}
...
int FastCodecCFFEL2_1_1_MarketData::EncodeData(void* marketDatas,int count)
{
  ...
  fastCodec->InitEncoder();
    ...
    fastCodec->EncodeDouble(FAST_OP_DEFAULT,15,item->settle		,4);
...
void* FastCodecCFFEL2_1_1_MarketData::DecodeData(uint8_t* bytes,int length,int& count)
{
...
int32_t wmdf::FastCodecCFFEL2_1_1_MarketData::SizeOfRecords( void* items,int count )
{
  return sizeof(CFEL2MarketSnapshot)*count;
}

void wmdf::FastCodecCFFEL2_1_1_MarketData::ResetValueDict()
{
  memset(main_DefaultValue,0,sizeof(struct tap_TValue)*CFFEL2_1_1_MARKETDATA_PMAP_BITS);
}
