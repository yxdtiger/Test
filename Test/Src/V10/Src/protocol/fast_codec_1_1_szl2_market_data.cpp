...
FastCodec_1_1_SZL2MarketData::FastCodec_1_1_SZL2MarketData()
{
  mainPMap=(Fast_PMap_T*)malloc(sizeof(struct Fast_PMap_T)+SZL2_MARKETDATA_1_1_PMAP_BITS*sizeof(uint8_t));
  memset(mainPMap,0,sizeof(struct Fast_PMap_T)+SZL2_MARKETDATA_1_1_PMAP_BITS*sizeof(uint8_t));
  mainPMap->size=SZL2_MARKETDATA_1_1_PMAP_BITS;

  main_DefaultValue = (TValue*)calloc(sizeof(struct tap_TValue), SZL2_MARKETDATA_1_1_PMAP_BITS);
}
...
int FastCodec_1_1_SZL2MarketData::EncodeData(void* items,int count)
{
  ...
  for (int i=0;i<count;i++)
  {
    ...
    fastCodec->EncodeDouble(FAST_OP_DEFAULT,51,item->peratio1	,4);
    fastCodec->EncodeDouble(FAST_OP_DEFAULT,52,item->peratio2	,4);
...
void* FastCodec_1_1_SZL2MarketData::DecodeData(uint8_t* bytes,int length,int& count)
{
  ...
  while(fastCodec->DecodeNewMsg(mainPMap,main_DefaultValue)>0)
  {
    ...
    fastCodec->DecodeDouble(FAST_OP_DEFAULT,51,currentItem.peratio1,4);
    fastCodec->DecodeDouble(FAST_OP_DEFAULT,52,currentItem.peratio2,4);	
...
int32_t wmdf::FastCodec_1_1_SZL2MarketData::SizeOfRecords( void* items,int count )
{
  return sizeof(L2MarketSnapshot)*count;
}

void wmdf::FastCodec_1_1_SZL2MarketData::ResetValueDict()
{
  memset(main_DefaultValue,0,sizeof(struct tap_TValue)*SZL2_MARKETDATA_1_1_PMAP_BITS);
}
