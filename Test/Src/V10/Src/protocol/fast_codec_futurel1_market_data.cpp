...
FastCodecFutureL1MarketData::FastCodecFutureL1MarketData()
{
	mainPMap=(Fast_PMap_T*)malloc(sizeof(struct Fast_PMap_T)+FUTUREL1_MARKETDATA_PMAP_BITS*sizeof(uint8_t));
	memset(mainPMap,0,sizeof(struct Fast_PMap_T)+FUTUREL1_MARKETDATA_PMAP_BITS*sizeof(uint8_t));
	mainPMap->size=FUTUREL1_MARKETDATA_PMAP_BITS;

  main_DefaultValue = (TValue*)calloc(sizeof(struct tap_TValue), FUTUREL1_MARKETDATA_PMAP_BITS);
}
...
int FastCodecFutureL1MarketData::EncodeData(void* marketDatas,int count)
{
  ...
  fastCodec->InitEncoder();
    ...
    fastCodec->EncodeDouble(FAST_OP_DEFAULT,14,item->average_price,4);//CZCE
    fastCodec->EncodeDouble(FAST_OP_DEFAULT,15,item->settle,4);		//CZCE
    fastCodec->EncodeDouble(FAST_OP_DEFAULT,16,item->pre_settle		,4);
    fastCodec->EncodeInt64(FAST_OP_DEFAULT,17,item->position);
    fastCodec->EncodeInt64(FAST_OP_DEFAULT,18,item->pre_position);
    fastCodec->EncodeDouble(FAST_OP_DEFAULT,19,item->high_limit		,4);
    fastCodec->EncodeDouble(FAST_OP_DEFAULT,20,item->low_limit		,4);
...
void* FastCodecFutureL1MarketData::DecodeData(uint8_t* bytes,int length,int& count)
{
    ...
    fastCodec->DecodeDouble(FAST_OP_DEFAULT,14,currentItem.average_price,4);
    fastCodec->DecodeDouble(FAST_OP_DEFAULT,15,currentItem.settle ,4);
    fastCodec->DecodeDouble(FAST_OP_DEFAULT,16,currentItem.pre_settle ,4);    
    fastCodec->DecodeInt64(FAST_OP_DEFAULT,17,currentItem.position);		 
    fastCodec->DecodeInt64(FAST_OP_DEFAULT,18,currentItem.pre_position);
    fastCodec->DecodeDouble(FAST_OP_DEFAULT,19,currentItem.high_limit ,4);  
    fastCodec->DecodeDouble(FAST_OP_DEFAULT,20,currentItem.low_limit ,4);   
...
int32_t wmdf::FastCodecFutureL1MarketData::SizeOfRecords( void* items,int count )
{
  return sizeof(FuturesMarketSnapshot)*count;
}

void wmdf::FastCodecFutureL1MarketData::ResetValueDict()
{
  memset(main_DefaultValue,0,sizeof(struct tap_TValue)*FUTUREL1_MARKETDATA_PMAP_BITS);
}
