...
namespace wmdf{
void TestSZL2MarketDataCodec()
	{
    cout<<"------SZSE L2 MarketData------"<<endl;
    double start, end;
    int times = 1,ilength=1000,bufferSize=0;
    L2MarketSnapshot* marketDatas = new L2MarketSnapshot[ilength];
    memset(marketDatas,0,sizeof(L2MarketSnapshot)*ilength);
		for(int i=0;i<ilength;i++)
		{
			string windCode = "000001.SH";
			memcpy(marketDatas[i].wind_code,windCode.c_str(),sizeof(marketDatas[i].wind_code));
      ...
      marketDatas[i].transactions_count =i*100001;


		}

		FastCodecSZL2MarketData* codec = new FastCodecSZL2MarketData();
    ...
    start = DateTime::now();
      ...
      std::cout<<"BufferSize:"<<bufferSize<<std::endl;
          ...
          for(int i = 0;i<count;i++)
          {
            ...
            {
              std::cout<<"FastCodecSZL2MarketData：["<<i<<"] error!"<<std::endl;
            }
           /* else
            {
