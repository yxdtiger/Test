...
void TestSHSZL1MarketDataBinaryCodec()
	{	
    cout<<"------SSE L1 MarketData BinaryCodec------"<<endl;
		int ilength=1000,times = 1,bufferSize=0;	
    double start, end;
    L1MarketSnapshot* marketDatas = new L1MarketSnapshot[ilength];
		...
		BinaryCodecSHSZL1MarketData* codec = new BinaryCodecSHSZL1MarketData();
    start = DateTime::now();
    ...
    std::cout<<"BufferSize:"<<bufferSize<<std::endl;
        ...
        for(int i = 0;i<count;i++)
        {
          ...
          {
            std::cout<<"BinaryCodecSHSZL1MarketData ["<<i<<"] error!"<<std::endl;
          }
          /*else
          {
