#include <iostream>
#include "fast_codec_shfel1_market_data.h"
#include "../include/wmdf_structs.h"

using namespace wmdf;

FastCodecSHFEL1MarketData::FastCodecSHFEL1MarketData()
{
	mainPMap=(Fast_PMap_T*)malloc(sizeof(struct Fast_PMap_T)+SHFEL1_MARKETDATA_PMAP_BITS*sizeof(uint8_t));
	memset(mainPMap,0,sizeof(struct Fast_PMap_T)+SHFEL1_MARKETDATA_PMAP_BITS*sizeof(uint8_t));
	mainPMap->size=SHFEL1_MARKETDATA_PMAP_BITS;

  main_DefaultValue = (TValue*)calloc(sizeof(struct tap_TValue), SHFEL1_MARKETDATA_PMAP_BITS);
}
...
int FastCodecSHFEL1MarketData::EncodeData(void* marketDatas,int length)
{
...
void* FastCodecSHFEL1MarketData::DecodeData(uint8_t* bytes,int length,int& count)
{
...
int32_t wmdf::FastCodecSHFEL1MarketData::SizeOfRecords( void* items,int count )
{
  return sizeof(FuturesMarketSnapshot)*count;
}

void wmdf::FastCodecSHFEL1MarketData::ResetValueDict()
{
  memset(main_DefaultValue,0,sizeof(struct tap_TValue)*SHFEL1_MARKETDATA_PMAP_BITS);
}
