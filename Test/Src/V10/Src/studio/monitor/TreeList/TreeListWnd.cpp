...
typedef struct
	...
	TCHAR		cToolTipText [260];							// Textspeicher für Tooltip
...
int				iRet;
	...
	if(uMsg==WM_KEYDOWN)
		...
		SendMessage(GetParent(hWnd),WM_COMMAND,MAKELONG(3,EN_SETTEXT),(LPARAM)hWnd);
		return iRet;
		}
...
unsigned	uSub;
	...
	if(iXpos>=pData->aColumnXpos[1])						// Auf Extraeintrag
		...
		for(uSub=1;uSub<pData->uColumnCount;uSub++)
			{
			if(iXpos>=pData->aColumnXpos[uSub+1])continue;
			...
			case DT_RIGHT:	iWidth  = pData->aColumnXpos[uSub+1];
			...
			case DT_CENTER:	iWidth  = pData->aColumnXpos[uSub+1];
							iWidth -= pData->aColumnXpos[uSub  ];
							iWidth += iIcon;
							iWidth /= 2;
...
int				i;
	...
	if(nChar>=' ')
		...
		if(TreeListStartNotifyEdit(pData,pData->uSelectedItem,uSub,nChar,lParam))
			{
			return;
			}
		...
		if(iMax<=0)return;
		   ...
		   iNum = pData->uSelectedItem;						// Holle die Startzeile
		if(iNum>0)
			{
...
int				iTemp;
	...
	switch(uBits&3)											// Editfenster neu erzeugen
		{
	case 1:	pData->hEdit=CreateWindow(_T("COMBOBOX"),NULL,WS_BORDER|WS_CHILD|CBS_AUTOHSCROLL|CBS_DROPDOWN,0,0,0,0,pData->hWnd,(HMENU)3,NULL,NULL);
			pData->uEditMode=1;
			SendMessage(pData->hEdit,CB_LIMITTEXT,2048,0);
			break;

	case 2:	pData->hEdit=CreateWindow(_T("COMBOBOX"),NULL,WS_BORDER|WS_CHILD|CBS_AUTOHSCROLL|CBS_DROPDOWNLIST,0,0,0,0,pData->hWnd,(HMENU)3,NULL,NULL);
			pData->uEditMode=2;
			SendMessage(pData->hEdit,CB_LIMITTEXT,2048,0);
			break;
	...
	pProc = (WNDPROC)GetWindowLongPtr(pData->hEdit,GWLP_WNDPROC);

	SetWindowLongPtr(pData->hEdit,GWLP_USERDATA,(LPARAM)pProc);
	...
	if(lRet&TVIR_EDITCOMBOBOX)								// Die Combobox füllen
		...
		if(sNotify.pTextList)								// Texte über Listenfeld
			...
			for(uCnt=0;uCnt<uMax;uCnt++)
				...
				if(!_tcscmp(cText,sNotify.item.pszText))
					{
					iSel = uCnt;
					}
...
HDC				hDc;
	...
	case WM_SIZE:												// Die Fenstergröße wurde verändert
			...
			if(uVal && uVal!=pData->uSizeY)
				...
				if(uFlag && pData->uPageEnties>2)				// Wenn Höhe vergrößert dann Scroll-Position prüfen
					{
	...
	case TVM_SETCOLUMN:											// Einen Spalten-Header einstellen
			...
			if(pData->hHeader && U(wParam)<pData->uColumnCount)
				...
				if(pCol->mask&TVCF_WIDTH){sItem.mask|=HDI_WIDTH;  sItem.cxy    =pCol->cx;pData->aColumn[U(wParam)].sSize = (short)sItem.cxy;}
...
int				i;
	...
	else	iMaxX = pData->aColumnXpos[1]-1;
...
NoRootLines:
		...
		if(pData->hImages && iImage!=TV_NOIMAGE)				// Icon zeichnen vom Haupteintrag
			{
			if(iImage>=0x40000000)								// Sub-Image-Liste verwenden
				{												
		...
		sArea.right = pData->aColumnXpos[1];					// Text ausgeben vom Haupteintrag
